...
describe("POST /companies", function () {
  ...
  test("doesn't work: regular non admin users", async function () {
    try {
      const resp = await request(app)
        .post("/companies")
        .send(newCompany)
...
describe("GET /companies", function () {
  ...
  // => only expect companies: c1 and c3
  test("GET /companies?maxEmployees", async function () {
  ...
  // => expect company: c2
  test("GET /companies?name", async function () {
  ...
  // test passing 2 parameters
  test("GET /companies?minEmployees&maxEmployees", async function () {
  ...
  test("GET /companies?minEmployees&maxEmployees&name", async function () {
    const resp = await request(app).get("/companies?minEmployees=1&maxEmployees=2&name=2");
    expect(resp.body).toEqual({
      companies:
        [
          {
  ...
  test("GET /companies?minEmployees&maxEmployees&name&somethingElse", async function () {
    try {
      await request(app)
        .get("/companies?minEmployees=1&maxEmployees=2&name=2&somethingElse=abc");
      // fail();
    } catch (err) {
      expect(err.status).toEqual(400);
      expect(err.message)
        .toEqual("instance is not allowed to have the additional property \"somethingElse\"");
    }
  ...
  test("GET /companies?minEmployees&somethingElse", async function () {
    try {
      await request(app).get("/companies?minEmployees=2&somethingElse=abc");
    } catch (err) {
      expect(err.status).toEqual(400);
      expect(err.message)
        .toEqual("instance is not allowed to have the additional property \"somethingElse\"");
